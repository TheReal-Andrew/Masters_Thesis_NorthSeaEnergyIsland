...
year       = 2030
mga_slack  = 0.0005   # MAA slack control
study_name = 'links_test'
...
# # List of country links that should NOT be be summed:
exclude_list =  [
                "Denmark",         
                "Norway",          
                "Germany",         
...
input_name        = 'v_2030_links_test_2_opt.nc'
# input_name        = 'case2_recreate.nc'
...
#%% Change carriers to select variables

# Create list of links to be summed based on links that are excluded from the sum
sum_list = [link for link in n.main_links if all(exclude_country not in link for exclude_country in exclude_list)]

# Set all 
for sum_link in sum_list: 
    n.links.loc[n.links.index == sum_link, 'carrier'] = 'link_sum'
    
# Check main links
# n.links.loc[n.main_links].carrier
...
while epsilon>MAA_convergence_tol:
    ...
    if len(solutions) <= 1:
        ...
        print(f'\n ### Directions in initial MGA search: {len(directions)} \n')
    else :
